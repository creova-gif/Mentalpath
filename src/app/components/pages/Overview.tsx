      ...
      <div className="grid grid-cols-[1.4fr_1fr] gap-5">
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl overflow-hidden mb-5">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
            <span className="text-sm font-medium text-[var(--ink)]">Today's sessions — Monday March 16</span>
          ...
          <div className="flex flex-col gap-0">
            {sessions.map((session, i) => (
              <div
                key={i}
                className={`flex items-center gap-3.5 px-5 py-3.5 border-t border-[var(--border)] cursor-pointer transition-all duration-100 hover:bg-[var(--warm)] ${
                  i === 0 ? 'border-t-0' : ''
                } ${session.status === 'now' ? 'bg-[var(--sage)]/[0.04]' : ''}`}
              >
                <div className="text-xs text-[var(--ink-muted)] min-w-[48px]">{session.time}</div>
                <div className={`w-[34px] h-[34px] rounded-full flex items-center justify-center text-xs font-medium flex-shrink-0 ${getAvatarColor(session.color)}`}>
                  {session.initials}
                </div>
                <div className="flex-1">
                  <div className="text-sm font-medium text-[var(--ink)]">{session.name}</div>
                  <div className="text-xs text-[var(--ink-muted)]">{session.type}</div>
                </div>
                {session.status === 'done' && (
                  <span className="text-[11px] bg-[#e8f4f0] text-[var(--sage-deep)] px-2 py-[3px] rounded font-medium">Done</span>
                )}
                {session.status === 'now' && (
                  <span className="bg-[var(--sage)] text-white text-[11px] px-2 py-[3px] rounded font-medium">Now</span>
                )}
        ...
        <div className="flex flex-col gap-5">
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl overflow-hidden">
            ...
            <div className="p-0">
              {taskList.map((task, i) => (
                <div key={i} className="flex items-start gap-2.5 px-5 py-3 border-t border-[var(--border)] first:border-t-0">
                  <div
          ...
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-4 pb-5">
            ...
            <div className="mt-2.5 text-xs text-[var(--ink-muted)]">$4,200 collected · $840 outstanding</div>
          </div>
        </div>
      </div>
...
function StatCard({ label, value, delta, negative }: { label: string; value: string; delta: string; negative?: boolean }) {
  return (
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-5">
      <div className="text-xs text-[var(--ink-muted)] font-medium uppercase tracking-[0.5px] mb-2">{label}</div>
      <div className="font-[var(--font-display)] text-[28px] text-[var(--ink)] mb-1">{value}</div>
      <div className={`text-xs ${negative ? 'text-[var(--red)]' : 'text-[var(--green)]'}`}>{delta}</div>
    </div>
  );
}
